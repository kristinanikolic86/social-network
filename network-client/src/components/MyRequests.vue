...
<html>
    <body>
      <router-link to="/user/me"><h3>Back To Profile</h3></router-link>
          ...
          <a class="btn" @click="acceptRequest(u.id)">Accept</a>
          <a class="btn" @click="declineRequest(u.id)">Decline</a>
          </div>
        </div>
    </div>
    </body>
...
export default class MyRequests extends Vue {
 ...
 myConfig = {
  headers : {"X-Auth-Token" : this.currentTokenAndUserInfo.token}
    ...
    getRequests(){
      axios.get('http://localhost:9000/api/requests/'+this.currentTokenAndUserInfo.user!.id, this.myConfig).then( res =>
      this.users = res.data
      )
    }

    acceptRequest(senderId:number){
            axios.get('http://localhost:9000/api/accept/'+senderId+'/'+this.id, this.myConfig).then( res =>
      this.$router.push('/user/'+senderId)
      )
    }

    declineRequest(senderId:number){
             axios.delete('http://localhost:9000/api/decline/'+senderId+'/'+this.id, this.myConfig).then( res =>
